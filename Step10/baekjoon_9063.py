...
# 예를 들어 위와 같이 (2, 1), (3, 2), (5, 2), (3, 4) 네 점에서 옥구슬을 발견하였다면, 임씨에게 돌아갈 대지는 (2, 1), (5, 1), (2, 4), (5, 4)를 네 꼭짓점으로 하는 직사각형이며, 
# 넓이는 (5 - 2) × (4 - 1) = 9 가 된다. 

def ground(n, coordinate_list):

    row = []
    col = []

    for i in range(n):
        row.append(coordinate_list[i][0])
        col.append(coordinate_list[i][1])

    total = (max(row) - min(row)) * (max(col) - min(col))
...
print(ground(n, coordinate_list))